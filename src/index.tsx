...
const IMAGE_SETS: Record<number, {
  ...
  3: {
    ...
    maskAreaDetail: '変更対象は画像手前に広がる芝生の広場です。背景の球場施設・照明塔・防球ネット・階段・空はそのまま保持してください。英語プロンプトでこのエリアを指す場合は必ず "the open grass lawn in the foreground" と呼ぶこと。mask / masked / white area / white region / white masked という語は一切使わないこと。背景の球場施設は、銀色の折板屋根を持つ観客スタンド、黒い照明塔、高い防球ネット、コンクリートの階段という同じ構成・同じ配置・同じ色調を保つこと。空は元画像と同じ青空を保つこと。背景の観客スタンド正面にある青い看板の「SGIスタジアム尼崎」の表示と、コンクリート壁面にある黄色い球団ショップのロゴ看板は、元画像と同じ位置・同じ見た目で残すこと。それ以外の球場名・チーム名・英字・ロゴ・広告文字を新たに描き加えることは禁止する。実在しない球場名（YOKOHAMA STADIUM など）を描いてはならない。英語プロンプトには必ず次の表現を含めること: keep the background stadium in the same layout, silver corrugated-metal grandstand roof, black floodlight poles, tall protective netting and concrete staircases; keep the existing blue signboard on the front of the grandstand and the small yellow team-shop logo sign on the concrete wall in the same position and appearance as the original; do not invent or add any other stadium name, team name, English lettering, logo or advertising text anywhere in the image; keep the same blue sky as the original。背丈の高い樹木は、背景の看板やロゴ看板の正面を避けて配置すること。ただし枝葉が自然に少し重なる程度は許容し、樹冠を途中で切り落とすことは絶対に禁止する。英語表現: place tall trees so that they do not block the signboards on the background stadium; slight natural overlap of branches is acceptable, but never cut off tree crowns。手前の地面は芝生の広場を基調とすること。新しく追加する施設のまわりに園路・小さな舗装面・遊具の足元の床を設けることは許容するが、手前の大部分は自然な芝生のまま残すこと。芝生の地盤の高さは元画像のまま平坦に保ち、地面を掘り下げた沈床広場・段状のコンクリート構造・擁壁を新たに作らないこと。ユーザーがコンクリート舗装・石畳・アスファルトなどの舗装を明示的に指定した場合を除き、手前を全面舗装で覆わないこと。なお「広場」という語だけでは舗装の指定とみなさないこと。英語表現: keep the foreground ground as a grass lawn; paths, small paved areas and the floor immediately around new facilities are acceptable, but the majority of the foreground must remain natural grass lawn; keep the existing flat ground level of the lawn and do not excavate the ground or create sunken plazas, stepped concrete structures or retaining walls; unless the user explicitly asks for paving such as concrete paving, stone paving or asphalt, do not cover the whole foreground with pavement; the Japanese word "広場" alone must not be treated as a request for paving',
