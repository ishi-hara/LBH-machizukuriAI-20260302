...
app.post('/api/refine-prompt', async (c) => {
  ...
  const systemPrompt =
    ...
    `- ただし、追加する建造物や樹木の上部は、マスク範囲の上端を自然に越えて描いてよい。マスク境界で水平に切り落とさないこと\n\n` +
    `ルール:\n` +
    ...
    `5. 新たに追加するすべての要素（建造物・樹木・植栽・門・塀などを含む）が、マスク範囲の境界や画像の端で不自然に途中で切れないよう指示する。樹木は幹から枝葉の先端まで、樹形全体が収まるように描くこと\n` +
    ...
    <div class="chat-section__messages" id="chatMessages" role="log" aria-live="polite" aria-label="チャット履歴">
      ...
      <div class="chat-message chat-message--ai">
        ...
        <div class="chat-message__bubble">
          こんにちは！${CURRENT_IMAGE_SET.maskAreaLabel}にどんな建造物を入れたいですか？（例：お城、神社、遊園地、水族館など）
        </div>
      </div>
