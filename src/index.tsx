...
app.post('/api/refine-prompt', async (c) => {
  ...
  const systemPrompt =
    ...
    `- 白い領域は「絵を描き込むための枠やキャンバス」ではありません。写真の一部を自然な風景に差し替えるための領域です。完成画像に白い縁・角丸の輪郭・境界線・枠が残ってはいけません\n\n` +
    `ルール:\n` +
    ...
    `8. マスクされた白いエリアのみを変更する指示にする\n` +
    `9. 新たに追加するすべての要素（施設・建造物・樹木・植栽・門・塀などを含む）が、マスク範囲の境界や画像の端で不自然に途中で切れないよう指示する。樹木や建物の上部は白い領域の形状に合わせて切り落とさず、背景の空や施設の手前に自然に重なってよい\n` +
