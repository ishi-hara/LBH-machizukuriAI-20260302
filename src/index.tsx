...
const IMAGE_SETS: Record<number, {
  ...
  3: {
    ...
    maskAreaDetail: '変更対象は画像手前に広がる芝生の広場です。背景の球場施設・照明塔・防球ネット・階段・空はそのまま保持してください。英語プロンプトでこのエリアを指す場合は必ず "the open grass lawn in the foreground" と呼ぶこと。mask / masked / white area / white region / white masked という語は一切使わないこと。背景の球場施設は、銀色の折板屋根を持つ観客スタンド、黒い照明塔、高い防球ネット、コンクリートの階段という同じ構成・同じ配置・同じ色調を保つこと。空は元画像と同じ青空を保つこと。球場や施設に文字・看板の文字・チーム名・ロゴ・英字を一切描かないこと。既存の看板は文字のない無地の面として描くこと。英語プロンプトには必ず次の表現を含めること: keep the background stadium in the same layout, silver corrugated-metal grandstand roof, black floodlight poles, tall protective netting and concrete staircases; do not render any text, lettering, team name, logo or signage text anywhere in the image, signboards must be blank; keep the same blue sky as the original。背丈の高い樹木は画像の左右端に寄せて配置する',
